...
More information about eigenda you can find here [eigenda-onboarding](https://github.com/eigenda-Network/BLS-TSS-Network/blob/main/docs/eigenlayer-onboarding.md).
...
```sh
helm install eigenda p2p-avs/eigenda -f values.yaml
```
...
1. Generate keys via the following URLs:
   ...
   - [eigenda Configs](https://github.com/0xJomo/eigenda-configs)
...
4. Run the following command to upgrade and install the chart:
   ```sh
   helm upgrade -i eigenda p2p-avs/eigenda -f values.yaml
   ```
...
send register-as-eigenlayer-operator /app/operator_keys/ecdsa_key.json $eigenda_NODE_ACCOUNT_KEYSTORE_PASSWORD
```

## Configuration
The following table lists the configurable parameters of the eigenda chart and their default values.
...
| `node.image.repository`     | Node image repository                                         | `ghcr.io/eigenda-network/node-client` |
...
| `nodeShell.image.repository` | nodeShell image repository                                     | `ghcr.io/eigenda-network/node-client` |
