# predicate Helm Chart

## Introduction

This repository contains a Helm chart for Kubernetes, specifically for the AVS named "predicate".  
More information about predicate you can find here [predicate-operator](https://github.com/predicateNetwork/predicate-operator).

## Table of Contents

- [predicate Helm Chart](#predicate-helm-chart)
...
```sh
helm install predicate p2p-avs/predicate -f values.yaml
```
...
1. Generate keys via the following URLs:
   ...
   - [predicate](https://github.com/predicateNetwork/predicate-operator)
...
4. Run the following command to upgrade and install the chart:
   ```sh
   helm upgrade -i predicate p2p-avs/predicate -f values.yaml
   ```

## Configuration

The following table lists the configurable parameters of the predicate chart and their default values.
...
| node.image.repository     | Node image repository                                 | ghcr.io/predicatenetwork/operator |
...
| register.image.repository | Register image repository                             | ghcr.io/predicatenetwork/operator |
